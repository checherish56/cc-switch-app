...
pub async fn check_for_updates(handle: AppHandle) -> Result<bool, String> {
    handle
        .opener()
        .open_url(
            "https://github.com/checherish56/cc-switch-app/releases/latest",
            None::<String>,
        )
...
fn try_get_version(tool: &str) -> (Option<String>, Option<String>) {
    ...
    #[cfg(target_os = "windows")]
    let output = {
        // Pre-check with `where` to avoid accidentally triggering protocol handlers
        let where_check = Command::new("cmd")
            .args(["/C", &format!("where {tool} 2>nul")])
            .creation_flags(CREATE_NO_WINDOW)
            .output();
        let found = where_check
            .as_ref()
            .map(|o| o.status.success() && !String::from_utf8_lossy(&o.stdout).trim().is_empty())
            .unwrap_or(false);
        if !found {
            // Try npm list -g as safe fallback for npm-based tools
            let npm_check = Command::new("cmd")
                .args(["/C", &format!("npm list -g @anthropic-ai/claude-code @openai/codex @google/gemini-cli --depth=0 2>nul")])
                .creation_flags(CREATE_NO_WINDOW)
                .output();
            let npm_has_tool = npm_check
                .as_ref()
...
pub async fn open_provider_terminal(
    ...
    cwd: Option<String>,
    launch_config: Option<crate::settings::ClaudeLaunchConfig>,
) -> Result<bool, String> {
    ...
    // 如果未传入配置，尝试读取默认配置
    let effective_config = launch_config.or_else(|| {
        crate::settings::get_claude_default_launch_config()
    });

    // 根据平台启动终端，传入提供商ID用于生成唯一的配置文件名
    launch_terminal_with_env(env_vars, &providerId, launch_cwd.as_deref(), effective_config)
        .map_err(|e| format!("启动终端失败: {e}"))?;

    Ok(true)
}

/// 将 ClaudeLaunchConfig 转换为 CLI 参数字符串
fn build_claude_cli_args(config: &crate::settings::ClaudeLaunchConfig) -> String {
    ...
    if let Some(ref effort) = config.effort {
        if !effort.trim().is_empty() {
            args.push(format!("--effort {}", effort.trim()));
        }
    }
    if let Some(ref mode) = config.permission_mode {
        if !mode.trim().is_empty() {
            args.push(format!("--permission-mode {}", mode.trim()));
        }
    }
    if config.verbose {
    ...
    if let Some(ref fmt) = config.output_format {
        if !fmt.trim().is_empty() {
            args.push(format!("--output-format {}", fmt.trim()));
        }
    }
    ...
    for dir in &config.add_dirs {
        if !dir.trim().is_empty() {
            args.push(format!("--add-dir \"{}\"", dir.trim()));
        }
    }
    if !config.allowed_tools.is_empty() {
        let tools = config.allowed_tools.iter()
            .map(|t| t.trim())
            .filter(|t| !t.is_empty())
            .collect::<Vec<_>>()
            .join(",");
        if !tools.is_empty() {
            args.push(format!("--allowedTools \"{}\"", tools));
        }
    }
    if !config.disallowed_tools.is_empty() {
        let tools = config.disallowed_tools.iter()
            .map(|t| t.trim())
            .filter(|t| !t.is_empty())
            .collect::<Vec<_>>()
            .join(",");
        if !tools.is_empty() {
            args.push(format!("--disallowedTools \"{}\"", tools));
        }
    }
    if let Some(ref sp) = config.system_prompt {
        if !sp.trim().is_empty() {
            args.push(format!("--system-prompt \"{}\"", sp.trim()));
        }
    }
    if let Some(ref asp) = config.append_system_prompt {
        if !asp.trim().is_empty() {
            args.push(format!("--append-system-prompt \"{}\"", asp.trim()));
        }
    }
...
fn launch_terminal_with_env(
    ...
    let cli_args = launch_config.as_ref()
        .map(|c| build_claude_cli_args(c))
        .unwrap_or_default();

    let config_path_str = config_file.to_string_lossy().to_string();
    let cli_cmd = format!("claude --settings \"{}\"{}", config_path_str, cli_args);

    #[cfg(target_os = "macos")]
    {
        launch_macos_terminal(&config_file, cwd, &cli_cmd)?;
        Ok(())
    }

    #[cfg(target_os = "linux")]
    {
        launch_linux_terminal(&config_file, cwd, &cli_cmd)?;
        Ok(())
    }

    #[cfg(target_os = "windows")]
    {
        launch_windows_terminal(&temp_dir, &config_file, cwd, &cli_cmd)?;
        return Ok(());
    }
...
/// macOS: 根据用户首选终端启动
#[cfg(target_os = "macos")]
fn launch_macos_terminal(config_file: &std::path::Path, cwd: Option<&Path>, cli_cmd: &str) -> Result<(), String> {
...
/// Linux: 根据用户首选终端启动
#[cfg(target_os = "linux")]
fn launch_linux_terminal(config_file: &std::path::Path, cwd: Option<&Path>, cli_cmd: &str) -> Result<(), String> {
    use std::os::unix::fs::PermissionsExt;
    use std::process::Command;
...
#[tauri::command]
pub async fn get_claude_default_launch_config(
) -> Result<Option<crate::settings::ClaudeLaunchConfig>, String> {
    Ok(crate::settings::get_claude_default_launch_config())
}

#[tauri::command]
pub async fn set_claude_default_launch_config(
    config: Option<crate::settings::ClaudeLaunchConfig>,
) -> Result<bool, String> {
    crate::settings::set_claude_default_launch_config(config)
        .map(|_| true)
