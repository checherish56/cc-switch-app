...
### Claude Code、Codex、Gemini CLI、OpenCode、OpenClaw & Hermes Agent 的全方位管理工具
...
**CC Switch** 为你提供一个桌面应用来管理所有六个 CLI 工具。无需手动编辑配置文件，你将获得一个可视化界面，一键将供应商导入应用，一键在不同的供应商之间进行切换，内置 50+ 供应商预设、统一的 MCP, SKILLS 管理以及系统托盘即时切换功能——所有操作都基于可靠的 SQLite 数据库和原子写入机制，保护你的配置不被损坏。

- **一个应用，六个 CLI 工具** — 在单一界面中管理 Claude Code、Codex、Gemini CLI、OpenCode、OpenClaw & Hermes Agent
...
浏览 **全部 5 个支持 Agent 的斜杠命令** — Claude Code（36 条）、Codex（33 条）、OpenCode（30 条）、OpenClaw（32 条）和 Hermes（32 条）。每个 Agent 拥有独立的分类可搜索参考面板，带有工具专属描述、CLI 等价标志和会话可用标记。一键复制，快速使用。

> **入口**：设置 → 关于 → "AI 编码工具斜杠命令教程" 按钮

### Codex、OpenCode、OpenClaw & Hermes 配置面板

为所有支持的 Agent 提供专属配置面板。每个 Agent 现在都拥有独立的设置标签页，用于管理供应商配置、启动选项和工具专属偏好。

### i18n 改进 — Agent 专属翻译

所有斜杠命令描述、记忆文件引用、账户登录和工具诊断现在使用 **按 Agent 独立的翻译键**。Codex 不再显示 "Claude Code" 或 "Anthropic" 相关内容，OpenCode 等工具在中、英、日三语中均显示各自的工具专属文本。
...
- **6 个 CLI 工具，50+ 预设** — Claude Code、Codex、Gemini CLI、OpenCode、OpenClaw、Hermes Agent；复制 key 即可一键导入
