...
import { ClaudeLaunchConfigDialog } from "@/components/ClaudeLaunchConfigDialog";
...
function App() {
  ...
  const [launchConfigDialog, setLaunchConfigDialog] = useState<{
    provider: Provider;
    cwd: string;
  } | null>(null);
  ...
  const handleOpenTerminal = async (provider: Provider) => {
    try {
      ...
      // Show config dialog before launching
      setLaunchConfigDialog({ provider, cwd: selectedDir });
    } catch (error) {
      console.error("[App] Failed to pick directory", error);
      const errorMessage = extractErrorMessage(error);
      toast.error(
  ...
  const handleLaunchWithConfig = async (config: ClaudeLaunchConfig) => {
    const info = launchConfigDialog;
    if (!info) return;

    try {
      await providersApi.openTerminal(info.provider.id, activeApp, {
        cwd: info.cwd,
        launchConfig: config,
      });
      toast.success(
  ...
  return (
    ...
    >
      ...
      <ClaudeLaunchConfigDialog
        open={launchConfigDialog !== null}
        cwd={launchConfigDialog?.cwd}
        onClose={() => setLaunchConfigDialog(null)}
