...
const CMD = (
  ...
  descriptionKey: `commands.openclaw.${id}.description`,
  usageKey: `commands.openclaw.${id}.usage`,
  sessionOnly: true,
...
export const OPENCLAW_COMMAND_CATEGORIES: Record<
  CommandCategory,
...
export const OPENCLAW_SLASH_COMMANDS: AgentSlashCommand[] = [
  // Session
  ...
  CMD("model", { category: "config", hasCliEquivalent: true }),
  CMD("permissions", { category: "config", hasCliEquivalent: true }),
  CMD("add-dir", {
    id: "add-dir",
    command: "/add-dir",
    category: "config",
...
export function openclawCommandsByCategory(): Map<
  CommandCategory,
  AgentSlashCommand[]
> {
  const map = new Map<CommandCategory, AgentSlashCommand[]>();
  for (const cmd of OPENCLAW_SLASH_COMMANDS) {
